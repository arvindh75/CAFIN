...
with open(directory + f"dist.pkl", "rb") as f:
...
class NeighborSampler(RawNeighborSampler):
    def sample(self, batch):
        ...
        for indx, u in enumerate(batch):
            ...
            if PRINT >= 2:
                print(
                    f"init pos -> u = {u}, v = {v}, dist = {dist_dc[u][v]}"
                )
            while dist_dc[u][v] != 1:
                v = u
            if PRINT >= 2:
                if v != prev_v:
                    print(f"Changed")
                print(
                    f"final pos -> u = {u}, v = {v}, dist = {dist_dc[u][v]}"
                )
            POS_DIST.append(dist_dc[u][v] / MAX_VAL)

            v = int(neg_batch[indx])
            prev_v = v
            if PRINT >= 2:
                print(
                    f"init neg -> u = {u}, v = {v}, dist = {dist_dc[u][v]}"
                )
            while dist_dc[u][v] <= NEG_MIN_DIST:
            ...
            if PRINT >= 2:
                if v != prev_v:
                    print(f"Changed")
                print(
                    f"final neg -> u = {u}, v = {v}, dist = {dist_dc[u][v]}\n"
                )
            NEG_DIST.append(dist_dc[u][v] / MAX_VAL)
