...
parser.add_argument("--dataset", type=str, help="The dataset2", default="Cora")
...
with open(directory + f"data.pkl", "rb") as f:
    dist_dc = pickle.load(f)
...
class NeighborSampler(RawNeighborSampler):
    def sample(self, batch):
        ...
        for indx, u in enumerate(batch):
            ...
            if PRINT >= 2:
                print(
                    f"init pos -> u = {u}, v = {v}, dist = {compute_distance(u, v, dist_dc)}"
                )
            while compute_distance(u, v, dist_dc) != 1:
                v = u
                # v = int(pos_batch[indx])
            if PRINT >= 2:
                if v != prev_v:
                    print(f"Changed")
                print(
                    f"final pos -> u = {u}, v = {v}, dist = {compute_distance(u, v, dist_dc)}"
                )
            POS_DIST.append(compute_distance(u, v, dist_dc) / MAX_VAL)

            v = int(neg_batch[indx])
            prev_v = v
            if PRINT >= 2:
                print(
                    f"init neg -> u = {u}, v = {v}, dist = {compute_distance(u, v, dist_dc)}"
                )
            while compute_distance(u, v, dist_dc) <= NEG_MIN_DIST:
            ...
            if PRINT >= 2:
                if v != prev_v:
                    print(f"Changed")
                print(
                    f"final neg -> u = {u}, v = {v}, dist = {compute_distance(u, v, dist_dc)}\n"
                )
            NEG_DIST.append(compute_distance(u, v, dist_dc) / MAX_VAL)
